...
def reward_idel_time(r, currentTime, machines):
    ...
    if r_next < r:
        ...
        if r_next == r:   # ref{Real-Time Scheduling for Dynamic Partial-No-Wait Multiobjective Flexible Job Shop by Deep Reinforcement Learning}
            reward = 0
        else:
            reward = -1
    return reward
        ...
        if not j.completed:
            ...
            for index in range(len(j.operation_list)):
                ...
                if not j.operation_list[index].completed:
                    ...
                    all_t_avg.append(t_avg)
            sum_t_avg = sum(all_t_avg)
            ETWF = j.weight * (max(T_cure, C_last) + sum_t_avg - j.RT)
        else:
            ETWF = j.weight * (j.endTime - j.RT)
        ETWFs.append(ETWF)
    return np.mean(ETWFs)

def machine_utilization(currentTime, machines):
    UKs = []
    for m in machines:
        pt = 0
...
def machine_utilization_ratio(sim):
    URs = []
    Cmax = []
    ...
    if r_WT_mean_next < r_WT_mean and r_WT_max_next < r_WT_max and r_WF_mean_next < r_WF_mean:
        ...
        if (r_WT_mean_next < r_WT_mean and r_WT_max_next < r_WT_max) or (
            ...
            if r_WT_mean_next < r_WT_mean or r_WT_max_next < r_WT_max or r_WF_mean_next < r_WF_mean:
                ...
                reward = -3
    reward = reward - (-1) / (3 - (-1)) # 为避免奖励函数倾斜，将奖励函数缩放到0到1之间 （x - min）/ (max - min)
...
def Controller_Reward(r, reward_task, currentTime, machines):
    UR_next = machine_utilization(currentTime, machines)
    if UR_next > r:
        reward_UR = 1
    else:
        if UR_next >= 0.97 * r:  # ref{Real-Time Scheduling for Dynamic Partial-No-Wait Multiobjective Flexible Job Shop by Deep Reinforcement Learning}
            reward_UR = 0
        else:
            reward_UR = -1
    reward = reward_UR + reward_task[0] + reward_task[1] + reward_task[2]
    return reward
...
def machine_utilizatioin_ratio(sim):
    Cmax = []
    PTs = []
    for m in sim.machines:
...
def global_Reward(r, pre_nas, currentTime, machines):
    UR_next = machine_utilization(currentTime, machines)

    if UR_next > r:
        reward_UR = 1
    else:
        if UR_next >= 0.90 * r:  # ref{Real-Time Scheduling for Dynamic Partial-No-Wait Multiobjective Flexible Job Shop by Deep Reinforcement Learning}
            reward_UR = 0
        else:
            reward_UR = -1
    num_all_assigned = num_assigned_jobs(machines)
    if pre_nas < num_all_assigned:
        reward_op = -10
    else:
        reward_op = num_all_assigned - pre_nas
    reward = reward_UR + reward_op
    return reward
